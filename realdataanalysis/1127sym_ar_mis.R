...
pars=scan(file = '1127_tsarpars.txt',what = "",skip = TASKID, nlines = 1)
#pars=c(40,15,2,0.01,0.01,0.01,0.01,2)
## SImulation dataset
phi_u0=pars[1]%>%as.numeric()
phi_x0=pars[2]%>%as.numeric() 
s_u0=pars[3]%>%as.numeric()
s_x0=pars[4]%>%as.numeric()
H_star=pars[5]%>%as.numeric()
a1=pars[6]%>%as.numeric()
a2=pars[7]%>%as.numeric()
set.seed(2)
...
future=proc.time()
future-now
save.image(paste0("tsar1127_mis_notau","_ID",TASKID,".RData"),compress = "xz")
